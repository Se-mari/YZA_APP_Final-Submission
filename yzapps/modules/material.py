...
def material_loadlist(pathname, searchterm):
    if pathname == '/modules/material':
        sql = """
        select material_name, category_name, unit_price, reorder_point, material_id
        from material ma
        ...
        values = [] 
        cols = ['Name', 'Category',  'Price per unit', 'Reorder point', 'ID'] #make sure column number of data matches sql
        ...
        if df.shape: 
            ...
            df['Action'] =buttons
            df= df[['Name', 'Category',  'Price per unit', 'Reorder point', 'Action']]
