...
layout = html.Div(
    [
        ...
        html.Div( 
                    [
                    dbc.Button('New Customer', color="secondary", href='/modules/customers_profile?mode=add'),
                    ]
                ),
        html.Hr(),
...
def purchaseprofile_populate(pathname, search):
    if pathname == '/modules/transaction_profile':
        ...
        removediv_style = {'display': 'none'} if not to_load or create_mode=='add' else None
        ...
        if eventid == 'transactionprofile_submit' and submitbtn:
            ...
            if not staff: 
                ...
                if create_mode == 'add':
                    ...
                    update transaction set lead_time=AGE(last_modified, created_date)::TEXT
                    where status_id=2;
                    """
